...
namespace DrawReborn
{
    public class DrawReborn
    {
        ...
        internal static float Arc(float x, float offset = 0f)
        {
            ...
            else if(offset < 0f)
            {
                ...
                return -Mathf.Sqrt(1 + (Harc_B * Harc_B) * Mathf.Pow(x - xC, 2f) / (Harc_A * Harc_A)) - Harc_C + offset;
            }
            else
            {
                // flattened hyperbola for top arc
                return -Mathf.Sqrt(1 + (Harc_B * Harc_B) * Mathf.Pow(x - xC, 2f) / (2f * Harc_A * Harc_A)) - Harc_C + offset;
            }
        }
        ...
        internal static List<Vector3> GetPositions(int N, float offset = 0f)
        {
            ...
            {
                maxX = Mathf.Clamp(maxX + 0.025f * (N - 5), maxX, 0.925f);
            }
        ...
        internal static Vector3 GetScale(int N)
        {
            ...
            {
                return new Vector3(1f, 1f, 1f) * factor * Mathf.Clamp(5f / (N / 2 + 2), 3f / 5f, 1f);
            }
            else
            {
                return new Vector3(1f, 1f, 1f) * factor * Mathf.Clamp(5f / (N - 1), 3f / 5f, 1f);
            }
    ...
    class CardChoicePatchSpawn
    {
        ...
        {
            __result.transform.localScale = DrawReborn.GetScale(PickReborn.PickReborn.startPick ? DrawReborn.startNumDraws : DrawReborn.numDraws);
        ...
        private static void RPCO_AddRemotelySpawnedCard(int viewID)
        {
            ...
            // set the scale
            card.transform.localScale = DrawReborn.GetScale(PickReborn.PickReborn.startPick ? DrawReborn.startNumDraws : DrawReborn.numDraws);
        }
    }
    ...
    class CardChoicePatchStartPick
    {
        ...
        private static void Prefix(CardChoice __instance)
        {
            ...
            List<Vector3> positions = DrawReborn.GetPositions(PickReborn.PickReborn.startPick ? DrawReborn.startNumDraws : DrawReborn.numDraws).WorldPoint();
            List<Quaternion> rotations = DrawReborn.GetRotations(PickReborn.PickReborn.startPick ? DrawReborn.startNumDraws : DrawReborn.numDraws);
            Vector3 scale = DrawReborn.GetScale(PickReborn.PickReborn.startPick ? DrawReborn.startNumDraws : DrawReborn.numDraws);
            ...
            for(int i = 1; i < (PickReborn.PickReborn.startPick ? DrawReborn.startNumDraws : DrawReborn.numDraws); i++)
            {
                GameObject newChild = UnityEngine.Object.Instantiate(cardVis, positions[i], rotations[i], __instance.transform);
